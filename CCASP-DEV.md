# CCASP-DEV.md - Development Guide for CCASP Package

This file provides guidance to Claude Code when working on the **Claude CLI Advanced Starter Pack (CCASP)** npm package itself.

> **Note**: This file is for CCASP development. The `CLAUDE.md` in this repo is a template that gets deployed to user projects.

---

## File Structure Separation

CCASP separates development files from deployed files:

| Folder/File | Purpose | Tracked in Git |
|-------------|---------|----------------|
| `.ccasp-dev/` | CCASP development hooks, settings, state | Yes (except state files) |
| `.claude/` | Deployed files (created by `ccasp init`) | No (gitignored) |
| `CCASP-DEV.md` | This file - CCASP dev instructions | Yes |
| `CLAUDE.md` | Template for deployed projects | Yes |
| `templates/` | Source templates for deployment | Yes |

### To develop CCASP:
1. Use the hooks in `.ccasp-dev/`
2. Modify source files in `src/`, `templates/`, `bin/`
3. Run `npm test` to verify syntax
4. The auto-build hook will remind you

### To test deployment:
1. Run `ccasp init --force` to regenerate `.claude/`
2. Restart Claude Code to load new commands
3. Test the deployed commands work correctly
...
- `.ccasp-dev/` - Development hooks and settings

This ensures the npm package remains functional. The PostToolUse hook `.ccasp-dev/hooks/ccasp-auto-build.js` will remind you if you forget.
...
.ccasp-dev/
├── settings.json         # Dev hook configuration
└── hooks/
    ├── ccasp-auto-build.js     # Build reminder hook
    └── ccasp-update-check.js   # Update check hook
```
