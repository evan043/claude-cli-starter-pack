@echo off
REM CCASP Neovide Demo Launcher - Path-hidden interactive demo for marketing screenshots
REM Same as ccasp-neovide.bat but with demo_mode=true to mask all file/folder paths.
REM Launches Claude CLI sessions in a clean temp directory so no real paths are visible.
REM Can run alongside the normal launcher.
...
REM Create a clean demo project directory (no real paths exposed)
set "DEMO_DIR=%TEMP%\ccasp-demo\my-project"
if not exist "%DEMO_DIR%\.claude" mkdir "%DEMO_DIR%\.claude"
if not exist "%DEMO_DIR%\.claude\settings.json" (
    echo {"permissions":{"allow":[]},"enabledTools":[]}> "%DEMO_DIR%\.claude\settings.json"
)
if not exist "%DEMO_DIR%\CLAUDE.md" (
    echo # My Project> "%DEMO_DIR%\CLAUDE.md"
    echo.>> "%DEMO_DIR%\CLAUDE.md"
    echo Demo project workspace.>> "%DEMO_DIR%\CLAUDE.md"
)
...
REM Start in the DEMO directory so Neovim's initial CWD is clean
REM "start /b" detaches the process so the cmd window closes immediately
REM demo_mode=true masks all paths in CCASP UI + redirects terminal sessions to demo dir
start "" /b /d "%DEMO_DIR%" "%NEOVIDE%" --frame none -- --clean -c "lua vim.opt.runtimepath:prepend('%PLUGIN_DIR:\=/%')" -c "lua require('ccasp').setup({layout='appshell', demo_mode=true})" -c "lua require('ccasp').open()"
