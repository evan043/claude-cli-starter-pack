 ...
 * Skips npm registry checks when dev mode is active (worktree linked).
 *
...
/**
 * Get global Claude path (cross-platform)
 */
...
function isDevModeActive() {
  try {
    const devStatePath = path.join(getGlobalClaudePath(), DEV_STATE_FILE);
    if (!fs.existsSync(devStatePath)) {
      return false;
    }
    const state = JSON.parse(fs.readFileSync(devStatePath, 'utf8'));
    return state.isDevMode === true;
  } catch {
    return false;
  }
}
...
module.exports = async function ccaspUpdateCheck(context) {
  // Skip npm checks entirely when dev mode is active
  // Dev mode uses worktree sync instead of npm updates
  if (isDevModeActive()) {
