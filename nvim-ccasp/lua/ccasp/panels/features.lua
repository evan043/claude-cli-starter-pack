...
function M.open()
  ...
  -- Create buffer
  M.bufnr = helpers.create_buffer("ccasp://features")

  -- Calculate position
  local pos = helpers.calculate_position({ width = 47, height = 35 })
...
function M.apply_highlights()
  local ns = helpers.prepare_highlights("ccasp_features", M.bufnr)
...
function M.setup_keymaps()
  ...
  local opts = helpers.setup_standard_keymaps(M.bufnr, M.winid, "Features", M, M.close)
