  ...
  pickers
    .new({}, {
      ...
      attach_mappings = function(prompt_bufnr, map)
        ...
        -- Custom action: Copy command name
        map("i", "<C-y>", function()
          local selection = action_state.get_selected_entry()
...
-- Browse all CCASP files
function M.files()
  local claude_dir = vim.fn.getcwd() .. "/.claude"
  if vim.fn.isdirectory(claude_dir) ~= 1 then
    vim.notify("CCASP: No .claude directory found", vim.log.levels.WARN)
    return
  end

  require("telescope.builtin").find_files({
    prompt_title = "CCASP Files",
    cwd = claude_dir,
    hidden = true,
  })
end

-- Search within CCASP configs
function M.grep()
  local claude_dir = vim.fn.getcwd() .. "/.claude"
  if vim.fn.isdirectory(claude_dir) ~= 1 then
    vim.notify("CCASP: No .claude directory found", vim.log.levels.WARN)
