...
test("get_default: returns complete config", function()
  ...
  assert_eq(false, cfg.features.competitor_research, "competitor_research default")
end)

test("get_default: has all 14 feature keys", function()
  local cfg = storage.get_default()
  local count = 0
  for _ in pairs(cfg.features) do count = count + 1 end
  assert_eq(14, count, "feature count")
end)
